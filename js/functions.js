...
function InitEditorWithEditorData(){
  ...
  //after initializing the editor we have to see if it is the user s first time coming to the site
  if(localStorage.getItem("user-first-time") == null){
    $("#promo-video-modal").modal('open');
  }
  localStorage.setItem("user-first-time", false);
}
  ...
  for(var i = 0; i < undefinedVars.length; i++){
    ...
    if(!IsVariableLatexStringVector(ls)){// there is a case where a variable is not a vector and it is undefined but we will see if we can find its vector counterpart and see if its vector counterpart is defined. If its vector counterpart is defined then we will add the vector magnitude which is the undefined variable we are currently parsing to where the vector is. It can be in either EL.undefinedvars.defined or DefinedVariables. 
      ...
      if(DefinedVariables[`\\vec{${ls}}`] != undefined){
        ...
        let savedUndefinedVariable = EL.retrieveSavedUndefinedVariablesData(ls);
        if(savedUndefinedVariable != undefined){
          props.state = savedUndefinedVariable.state;
          props.value = savedUndefinedVariable.value;
          // if we are editing the properties based on past data that the user gave the variable then the variable shouldn't be autoGenerated any more because the user had something to do with setting the data and it wasn't all set by the editor
          autoGenerated = false;
        }
        ...
        UpdateDefinedVariables({
          ...
          autoGenerated: autoGenerated,// setting this variable equal to autogenerated because the user didn't define this undefined variable we did so if the user stops using the variable in their work it should just go away
        });
        ...
        // this variable may have some saved data on it like its state and its value. We will only worry about the state because "EL.recordDefinitionForUndefinedVariable" also does its own search for previous data on this variable so the value will persist if it isn't undefined
        let savedUndefinedVariable = EL.retrieveSavedUndefinedVariablesData(ls);
        if(savedUndefinedVariable != undefined){
          props.state = savedUndefinedVariable.state;
        }

        EL.recordDefinitionForUndefinedVariable(ls, props.unitsMathjs, props.state, props.currentState, props.value);
